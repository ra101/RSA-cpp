...
public:
    ...
    template<typename msgClass> std::size_t sign(msgClass message); 
};
...
template<typename msgClass>bool verify(msgClass,std::size_t,unsigned long long int);
...
template<typename msgClass> std::size_t RSAKeyPair::sign(msgClass message)
{
    std::hash<msgClass> hashFuntion;
...
bool verify(msgClass message,std::size_t digest,unsigned long long int othersPublicKey)
    {
    std::hash<msgClass> hashFuntion;
    short int hashOutput =  hashFuntion(message);
    short int res = encript(digest,othersPublicKey);
